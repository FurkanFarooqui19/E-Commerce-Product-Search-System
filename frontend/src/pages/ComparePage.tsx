...
export const ComparePage: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      {/* Input Bar & Controls */}
      <div className="max-w-4xl mx-auto glass-panel rounded-3xl p-5 border border-border shadow-glass space-y-4">
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <div className="relative flex-1 w-full">
            <input
              ...
              className="w-full py-3 px-4 bg-surface-well border border-border rounded-2xl text-sm text-white placeholder-slate-400 focus:outline-none focus:border-indigo-500 font-sans font-medium"
            />
          </div>

          <div className="flex items-center space-x-2 w-full sm:w-auto">
            {/* Top-K Selector */}
            <div className="flex items-center space-x-1 bg-surface-muted border border-border rounded-2xl px-2.5 py-1.5 text-xs">
            ...
            <button
              ...
              disabled={loading}
              className="flex-1 sm:flex-initial px-6 py-3 bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-500 hover:to-indigo-400 text-white font-display font-bold text-xs rounded-2xl shadow-lg shadow-indigo-600/30 flex items-center justify-center space-x-2 transition-all disabled:opacity-50 flex-shrink-0"
            >
        ...
        <div className="flex flex-wrap items-center gap-1.5 pt-1 text-xs">
          <span className="text-slate-400 font-mono text-[11px] mr-1">Curated Test Queries:</span>
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-2 sm:grid-cols-4 gap-3">
          {modesConfig.map(({ id, title, accent, indicator }) => {
            ...
            return (
              <div
                key={id}
                className="glass-panel p-4 rounded-2xl border border-border flex items-center justify-between shadow-glass"
              >
                <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 items-stretch">
          {modesConfig.map(({ id, title, badge, color, accent, borderHover, desc }) => {
            ...
            return (
              <div
                key={id}
                className={`glass-panel rounded-3xl p-5 border border-border flex flex-col justify-between space-y-4 transition-all duration-300 ${borderHover}`}
              >
                {/* Column Header */}
                <div>
                  ...
                  <p className="text-[11px] text-slate-300 leading-snug mb-4 min-h-[36px] font-sans">
                    {desc}
                  </p>
                    ...
                    {results.length > 0 ? (
                      results.map((item, idx) => {
                        return (
                          <div
                            key={idx}
                            className="p-3.5 rounded-2xl bg-surface-well border border-border hover:border-indigo-500/40 transition-all space-y-2 group shadow-inner"
                          >
                            <div className="flex items-center justify-between text-xs">
                              <span className="flex items-center justify-center h-5 px-1.5 rounded-md bg-surface-muted border border-border text-[11px] font-mono font-bold text-slate-300">
                                #{item.rank}
                              </span>
                            ...
                            <h4 className="font-display font-semibold text-xs text-slate-100 line-clamp-2 leading-snug group-hover:text-indigo-300 transition-colors">
                              {item.product_name}
                            </h4>

                            <div className="flex items-center justify-between text-[10px] text-slate-400 pt-1.5 border-t border-border font-mono">
                      ...
                      <div className="p-8 rounded-2xl bg-surface-well/50 text-center text-xs text-slate-400 font-mono">
                        No matches found
                      </div>
                    )}
                  </div>
                ...
                {/* Footer notes */}
                <div className="pt-3 border-t border-border text-[11px] text-slate-400 font-mono text-center flex items-center justify-center space-x-1.5">
      ...
      <div className="glass-panel p-6 rounded-3xl border border-border shadow-glass space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs text-slate-300 leading-relaxed pt-1 font-sans">
          <div className="p-4 rounded-2xl bg-surface-well border border-border space-y-1 shadow-inner">
            <span className="font-bold text-emerald-300 font-display block">BM25 Saturation Curve</span>
          ...
          <div className="p-4 rounded-2xl bg-surface-well border border-border space-y-1 shadow-inner">
            <span className="font-bold text-purple-300 font-display block">TF-IDF Vector Space</span>
            <p className="text-slate-400">
          ...
          <div className="p-4 rounded-2xl bg-surface-well border border-border space-y-1 shadow-inner">
            <span className="font-bold text-amber-300 font-display block">Hybrid Ranker Synergy</span>
            <p className="text-slate-400">
