...
export const Navbar: React.FC<NavbarProps> = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-border bg-surface-well/85 backdrop-blur-xl transition-all shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        >
          <div className="relative h-10 w-10 rounded-xl bg-gradient-to-br from-indigo-500 via-purple-600 to-cyan-500 p-[1.5px] shadow-glow group-hover:shadow-indigo-500/50 transition-all duration-300">
            <div className="h-full w-full bg-surface-well rounded-[10px] flex items-center justify-center">
          ...
          <div>
            <div className="flex items-center space-x-2">
              ...
              <span className="text-[10px] uppercase font-mono font-bold tracking-widest px-2 py-0.5 rounded-md bg-indigo-500/15 text-indigo-300 border border-indigo-500/30">
                IR CORE
              </span>
            </div>
            <p className="text-[11px] text-slate-400 hidden sm:flex items-center space-x-1.5 font-mono">
              <span className="text-emerald-400 font-semibold">BM25</span>
              <span className="text-slate-600">•</span>
              <span className="text-amber-400 font-semibold">Hybrid</span>
              <span className="text-slate-600">•</span>
              <span className="text-purple-400 font-semibold">TF-IDF</span>
              <span className="text-slate-600">•</span>
        ...
        {/* Navigation Tabs */}
        <nav className="flex items-center space-x-1 bg-surface-muted/90 p-1 rounded-2xl border border-border shadow-inner">
          {navItems.map(({ id, label, icon: Icon, shortcut }) => {
            ...
            return (
              <button
                ...
                className={`relative flex items-center space-x-2 px-3.5 py-1.5 rounded-xl text-xs font-display font-semibold transition-all duration-200 ${
                  isActive
                ...
                <span
                  className={`hidden xl:inline text-[9px] font-mono px-1 py-0.2 rounded border ${
                    isActive
                      ...
                      : "bg-surface-well border-border text-slate-500"
                  }`}
                >
                  {shortcut}
                </span>
        ...
        <div className="flex items-center space-x-2.5">
          {health && !error ? (
            <div className="flex items-center space-x-2 px-3 py-1.5 rounded-full bg-surface-muted border border-border text-xs shadow-sm">
