...
def test_custom_stopword_removal(preprocessor):
    ...
    assert preprocessor.process("best cheap wireless headphones") == ["wireless", "headphon"]
...
def test_punctuation_split(preprocessor):
    assert preprocessor.process("wireless, bluetooth-enabled; headphones.") == ["wireless", "bluetooth", "enabl", "headphon"]
