...
from app.api.schemas.request import EvaluationRequest  # noqa: E402
from app.models.evaluation import EvaluationQuery, RelevanceJudgment  # noqa: E402
from app.models.index import IndexStore  # noqa: E402
from app.engine.bm25_ranker import BM25Ranker  # noqa: E402
from app.engine.filter_engine import FilterEngine  # noqa: E402
from app.engine.preprocessor import QueryPreprocessor  # noqa: E402
from app.engine.result_fusion import ResultFusion  # noqa: E402
...
"""
    ...
    c2.outputs = [
        new_output(
            "stream",
            name="stdout",
            text="Inverted index loaded successfully. Ready to serve requests.\n",
        )
    ]
...
"""
    ...
    with SessionLocal() as db:
        ...
        eval_req = EvaluationRequest(
            query_set_id=1, modes=["keyword", "tfidf", "bm25", "hybrid"], k=10
        )
        eval_response = EvaluationService.run(eval_req, db)
    report = eval_response["evaluation_report"]
    ...
    c3.outputs = [
        new_output(
            "stream",
            name="stdout",
            text=f"Evaluated {report['total_queries']} queries at k={report['k']}. Winner: {report['winner']}\n",
        )
    ]
...
"""
    ...
    df_metrics.plot(kind="bar", ax=axes[0], rot=0, colormap="viridis")
    axes[0].set_title(
        "Information Retrieval Quality (k=10)", fontsize=14, fontweight="bold"
    )
    axes[0].set_ylabel("Score [0.0 - 1.0]")
    axes[0].set_ylim(0.5, 0.85)
    for p in axes[0].patches:
    ...
    df_mrr.plot(kind="bar", ax=axes[1], rot=0, colormap="magma")
    axes[1].set_title(
        "Ranking & Coverage (MRR & Recall@10)", fontsize=14, fontweight="bold"
    )
    axes[1].set_ylabel("Score [0.0 - 1.0]")
    axes[1].set_ylim(0.3, 1.0)
    for p in axes[1].patches:
...
"""
    ...
    ax.set_title(
        "Per-Query NDCG@10 Comparison (Selected Queries)",
        fontsize=14,
        fontweight="bold",
    )
    ax.set_ylabel("NDCG@10")
    ...
    c8 = new_markdown_cell(
        f"""## Conclusion & Key Findings
...
"""
    ...
    with SessionLocal() as db:
        ...
        for k1, b in itertools.product(k1_vals, b_vals):
            ...
            for q in queries:
                tokens = preprocessor.process(q.query_text)
                candidates = FilterEngine.get_candidate_ids(
                    q.category, q.min_price, q.max_price, db
                )
                scored = BM25Ranker.rank(
                    tokens, candidates, store.index, store.corpus_stats, k1=k1, b=b
                )
                meta = SearchService._build_product_meta(scored, db)
                fused = ResultFusion.normalize_and_sort(
                    scored, tokens, store.index, meta
                )
                top_k_ids = [pid for pid, _ in fused[:10]]

                judgments = (
                    db.query(RelevanceJudgment)
                    .filter(RelevanceJudgment.query_id == q.id)
                    .all()
                )
                grades = {j.product_id: j.relevance for j in judgments}
                rel_ids = {j.product_id for j in judgments if j.relevance >= 2}
            ...
            avg_p = sum(p_scores) / len(p_scores)
            grid_records.append(
                {"k1": k1, "b": b, "NDCG@10": avg_ndcg, "Precision@10": avg_p}
            )
    df_grid = pd.DataFrame(grid_records)
    ...
    c3.outputs = [
        new_output(
            "stream",
            name="stdout",
            text=f"Running grid search over 4 x 3 = 12 configurations on {len(queries)} queries...\nGrid search execution complete.\n",
        )
    ]
...
"""
    ...
    c4.outputs = [
        new_output(
            "stream", name="stdout", text="NDCG@10 Parameter Matrix (k1 vs b):\n"
        ),
        new_output(
            "execute_result",
            data={
...
"""
    ...
    axes[1].set_title(
        "BM25 Parameter Grid: Precision@10", fontsize=13, fontweight="bold"
    )
    axes[1].set_xlabel("Document Length Normalization (b)", fontsize=11)
    axes[1].set_ylabel("Term Frequency Saturation (k1)", fontsize=11)
...
"""
    ...
    c7 = new_markdown_cell(
        f"""## Parameter Analysis & Discussion
